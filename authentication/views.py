...
class UserDetailAPIView(APIView):
    def get(self, request, user_id, *args, **kwargs):
        logger.debug(f"Fetching information for user_id: {user_id}")
        user = get_object_or_404(User, id=user_id)
        serializer = UserSerializer(user)
        logger.debug(f"Successfully fetched information for user_id: {user_id}")
...
class RegisterView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        send_email('Verify your email', user.email, email_body)
...
class RequestPasswordResetEmail(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if User.objects.filter(email=email).exists():
            ...
            send_email('Reset your password', user.email, email_body)
